...
import (
	...
	"github.com/nchuxyz/firefly/client"
...
type fireflyClient struct {
	...
	socksHandler   *client.RelayHandler
...
func (c *fireflyClient) changeRelayHandler() {
	...
	newHandler := &client.RelayHandler{
...
func (c *fireflyClient) parseOptions() {
	flag.BoolVar(&c.options.cmdMode, "cmd-mode", true, "command line mode")
	...
	flag.StringVar(&c.options.tunnelListFile, "tunnels", "", "a text file contains a list of tunnels")
...
func (c *fireflyClient) _main() {
	...
	handler := &client.TunnelHandler{
		AppData: c.appData,
		ReqCh:   make(chan *client.TunnelRequest),
		QuitCh:  make(chan bool),
		Auth:    sockstun.NewTunnelAnonymousAuthenticator(),
	}
	if c.options.tunnelListFile == "" {
		log.Printf("FATAL: option 'tunnels' is required")
		os.Exit(1)
	}
	err = handler.LoadTunnelPeers(c.options.tunnelListFile)
	if err != nil {
	...
	// start SOCKS proxy
	domains := c.loadTunnellingDomains()
	c.socksHandler = &client.RelayHandler{
